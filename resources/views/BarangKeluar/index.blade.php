...
<script>
    $(document).on('click', '.open_modal_show', function() {
        ...
        $.get(url, function(data) {
            ...
            $('#unit').text(data.unitKeluar);
            $('#device').text(data.device);
            $('#merek').text(data.merek);
            if (data.gambar) {

                $('#image-bar').attr('src', "{{ Storage::url('public/ImagesBarangKeluar/') }}" + data.gambar);
            } else {
    ...
    window.addEventListener('openmodal', event => {
        $("#ModalInputSN").modal('show');

    })
    window.addEventListener('closemodal', event => {
        $("#ModalInputSN").modal('hide');
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="card mb-3 border-0" style="max-width: 540px;">
                    <div class="row g-0">
                        <img src="" id="image-bar" class="card-img-top">
                        <div>
                            <div class="card-body">
